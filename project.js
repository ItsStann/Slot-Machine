...
// 3. Collect a certain amount of bet
// 4. Spin the slot machine
// 5. Check if the user won or not
// 6. Award their winnings if they won
// 7. Play again or if the user runs out of money
...
const getNumberOfLines = () => {
    while (true) {
        ...
        if (isNaN(numberOfLines) || numberOfLines <= 0 || numberOfLines > 3) {
            console.log("Invalid lines to bet on. Please try again.");
        }
        else {
...
const getBet = (balance, numberOfLines) => {
    while (true) {
        ...
        if (isNaN(numberBet) || numberBet <= 0 || numberBet > balance) {
            ...
            const splitBet = (numberBet/numberOfLines);
            console.log(`You are betting a total amount of $${numberBet} on ${numberOfLines} lines. So $${splitBet.toFixed(2)} per line`);
            return splitBet;
        }
    }
};
